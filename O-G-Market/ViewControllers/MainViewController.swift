...
final class MainViewController: UIViewController {
	...
	override func viewWillAppear(_ animated: Bool) {
		...
		if let indexPath = self.productCollectionView.indexPathsForSelectedItems?.first {
			if let coordinator = self.transitionCoordinator {
				coordinator.animate { _ in
					self.productCollectionView.deselectItem(at: indexPath, animated: true)
				} completion: { context in
					if context.isCancelled {
						self.productCollectionView.selectItem(at: indexPath, animated: false, scrollPosition: [])
					}
				}
			} else {
				self.productCollectionView.deselectItem(at: indexPath, animated: animated)
			}
		}
	}
	...
	private func createListCellRegistration() -> UICollectionView.CellRegistration<ListItem, Int> {
		return UICollectionView.CellRegistration<ListItem, Int> { cell, indexPath, item in
			cell.productImageView.image = UIImage(systemName: "bolt.car")
			cell.productNameLabel.text = "MacBook Pro 14"
			cell.productDiscountedPriceLabel.text = "3,000,000"
			cell.productPriceLabel.text = "3,500,000"
